...
function articlesMedPublishedByRegion($annee) {
...
curl_setopt($ch, CURLOPT_URL, 'http://localhost:81/API_JSON_PHP/stats/articlesMed/published/byRegion.php');
...
// echo '<br>';
		while ($i<$count){
			    ...
			    $resultabscisse.="\"" . $json["results"][$i]["region"] . "\"";
		...
		return array("nb"=>$resultstr,"region"=>$resultabscisse);

}

function articlesMedPublishedByRegionStats($annee) {
	global $token_security;
...
//Set the URL that you want to GET by using the CURLOPT_URL option.
curl_setopt($ch, CURLOPT_URL, 'http://localhost:81/API_JSON_PHP/stats/articlesMed/published/byRegionStats.php');
