...
function articlesReadByMostRead($annee) {
global $token_security;
 //Initialize cURL.
 $ch = curl_init();

 //Set the URL that you want to GET by using the CURLOPT_URL option.
 curl_setopt($ch, CURLOPT_URL, 'http://localhost:81/API_JSON_PHP/stats/articles/read/byMostRead.php');
...
// var_dump($data);
 ...
 // echo '<br>';
 	 while ($i<$count){
 			 if ($next) { $resultstr .= ","; $resultabscisse .= ","; }
 				 ...
 				 $resultabscisse.="\"" . $json["results"][$i]["title"] . "\"";
 	 ...
 	 return array("nb"=>$resultstr,"title"=>$resultabscisse);

 }

 function articlesReadByMostReadStats($annee) {
 	global $token_security;
 ...
 //Set the URL that you want to GET by using the CURLOPT_URL option.
 curl_setopt($ch, CURLOPT_URL, 'http://localhost:81/API_JSON_PHP/stats/articles/read/byMostReadStats.php');
