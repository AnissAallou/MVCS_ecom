...
function articlesPublishedByTag($annee) {
...
curl_setopt($ch, CURLOPT_URL, 'http://localhost:81/API_JSON_PHP/stats/articles/published/byTag.php');
...
// echo '<br>';
		while ($i<$count){
			    ...
			    $resultabscisse.="\"" . $json["results"][$i]["tag"] . "\"";
		...
		return array("nb"=>$resultstr,"tag"=>$resultabscisse);

}

function articlesPublishedByTagStats($annee) {
	global $token_security;
...
//Set the URL that you want to GET by using the CURLOPT_URL option.
curl_setopt($ch, CURLOPT_URL, 'http://localhost:81/API_JSON_PHP/stats/articles/published/byTagStats.php');
